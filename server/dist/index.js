...
const exercisesCount = db.prepare('SELECT COUNT(*) as count FROM exercises').get();
...
app.get('/api/v1/counter', (req, res) => {
    const row = db.prepare('SELECT remaining_count FROM global_counter WHERE id = 1').get();
    res.json({
        success: true,
        data: {
...
app.get('/api/v1/exercises', (req, res) => {
    const rows = db.prepare('SELECT * FROM exercises ORDER BY created_at').all();
    res.json({
        success: true,
        data: rows.map(r => ({
            id: r.id,
...
app.get('/api/v1/users', (req, res) => {
    const q = req.query.q || '';
    const rows = db.prepare('SELECT id, name FROM users WHERE name LIKE ? ORDER BY name LIMIT 10').all(q + '%');
    res.json({
        success: true,
        data: rows
    });
...
app.post('/api/v1/workouts', (req, res) => {
    ...
    try {
        const transaction = db.transaction(() => {
            ...
            const userCount = db.prepare('SELECT amount FROM user_exercise_counts WHERE user_id = ? AND exercise_id = ?').get(user.id, exercise.id);
            if (userCount) {
            ...
            const remainingRow = db.prepare('SELECT remaining_count FROM global_counter WHERE id = 1').get();
            return {
                userId: user.id,
...
app.post('/api/v1/exercise-requests', (req, res) => {
    ...
    const id = 'req_' + (0, uuid_1.v4)().slice(0, 8);
    db.prepare(`
...
if (process.env.NODE_ENV === 'production') {
    const staticPath = path_1.default.join(__dirname, 'site-dist');
