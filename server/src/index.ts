...
app.post('/api/v1/workouts', (req, res) => {
    ...
    try {
        const transaction = db.transaction(() => {
            ...
            db.prepare('INSERT INTO workout_logs (id, user_id, exercise_id, input_amount, counter_delta, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(workoutId, user.id, exercise.id, amount, counterDelta, Date.now());
            ...
            if (userCount) {
                db.prepare('UPDATE user_exercise_counts SET amount = amount + ? WHERE user_id = ? AND exercise_id = ?').run(amount, user.id, exercise.id);
            } else {
                db.prepare('INSERT INTO user_exercise_counts (user_id, exercise_id, amount) VALUES (?, ?, ?)').run(user.id, exercise.id, amount);
            }
